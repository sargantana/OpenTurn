...
#endif //defined (CY_USING_HAL)
#if defined (CY_USING_HAL)
	const cyhal_resource_inst_t CLK_DRV_SPI_obj = 
	{
		.type = CYHAL_RSC_CLOCK,
		.block_num = CLK_DRV_SPI_HW,
		.channel_num = CLK_DRV_SPI_NUM,
	};
#endif //defined (CY_USING_HAL)
#if defined (CY_USING_HAL)
...
void init_cycfg_clocks(void)
{
	...
	Cy_SysClk_PeriphSetDivider(CY_SYSCLK_DIV_8_BIT, 1U, 14U);
	Cy_SysClk_PeriphEnableDivider(CY_SYSCLK_DIV_8_BIT, 1U);
#if defined (CY_USING_HAL)
	cyhal_hwmgr_reserve(&CLK_DRV_SPI_obj);
#endif //defined (CY_USING_HAL)

	Cy_SysClk_PeriphDisableDivider(CY_SYSCLK_DIV_8_BIT, 2U);
	Cy_SysClk_PeriphSetDivider(CY_SYSCLK_DIV_8_BIT, 2U, 14U);
